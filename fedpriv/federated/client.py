...
# contribute an unbounded update. DP NOISE, however, is added centrally by
# the SERVER once per round, after aggregation (see federated/server.py) —
# this is the standard "DP-FedAvg" construction of McMahan et al. (2018,
# "Learning Differentially Private Recurrent Language Models"). Adding one
# correctly-calibrated noise draw to the aggregate (sensitivity = C /
# cohort_size) is far more sample-efficient than every client
# independently adding full-sensitivity noise before averaging, which is
# why we don't do the latter: the achievable privacy-utility tradeoff at a
# fixed epsilon is dramatically better this way, and it's what real DP-FL
# systems deploy.
# """
...
#     def local_train(self, global_params: np.ndarray, model: NumpyMLP) -> ClientUpdateResult:
